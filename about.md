...
*   **Shiori からの回答**:
    ...
    > *   類似インシデント: [GitHub Issue #390 (package_install / editable install の uv 優先を $VIRTUAL_ENV 条件付きで再有効化する)](https://github.com/masuda-masuo/sunaba/issues/390)
...
1.  **コードの取得**:
    ...
    *   ➔ **取得した実コード**:
        ...
        ```
2.  **過去の知見の取得**:
    *   `shiori_read_issue(number=390)`
    *   ➔ **取得した設計背景**: `"venvレスなカスタムイメージでは従来どおり pip の user-site フォールバックに落とし、標準イメージ（venv焼き込み済み）では uv で高速にインストールさせる"`
